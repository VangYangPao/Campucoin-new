...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CMPCOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CMPCOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CMPCOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cmpco-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CMPCOMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CMPCOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCMPCOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCMPCOMiner() exception");
    }

    LogPrintf("ThreadCMPCOMiner exiting\n");
}
